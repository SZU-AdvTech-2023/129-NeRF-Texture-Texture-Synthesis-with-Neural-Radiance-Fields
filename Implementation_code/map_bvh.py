...
class BvhMeshProjector:
    def __init__(self, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), mesh_path=None, mesh=None,
                 mean_edge_length=None, compute_normals=True, store_f=False, store_uv=False, **kwargs):
        # Initialize mesh
        if mesh is None:
    ...
    def barycentric_mapping(self, xyz, h_threshold=None, sdf_scale=1., sdf_offset=0., requires_grad_xyz=False,
                            return_face_id=False, **kwargs):
        sdf, fid, barycentric = self.bvh.signed_distance(xyz, return_uvw=True, mode='raystab')
        sdf = sdf / sdf_scale - sdf_offset
        sdf = sdf.unsqueeze(-1)

...
class patchBasedTextureSynthesis:
    ...
    def __init__(self, patches, in_outputPath, in_outputSize, in_patchSize, in_overlapSize, in_windowStep=5,
                 in_mirror_hor=True, in_mirror_vert=True, in_shapshots=True, rotate=True, picked_vertices=None,
                 patch_length=None, coarse_KDtree=True, match_dim=None, sample_tbn=None, strict_match=False,
                 mode='Cut'):
    ...
    def findMostSimilarPatches(self, overlapArea_Top, overlapArea_Left, coord, in_k=16):
        if self.coarse_KDtree:
            overlapArea_Top = self.coarse_top_func(
                overlapArea_Top[..., :self.match_dim]) if overlapArea_Top is not None else None
            overlapArea_Left = self.coarse_left_func(
                overlapArea_Left[..., :self.match_dim]) if overlapArea_Left is not None else None
    ...
    def initKDtrees(self):
        ...
        if self.coarse_KDtree:
            ...
            self.coarse_top_func = lambda overlap: block_reduce(overlap, (
            1, 1, block_size, 1)) if overlap.ndim == 4 else block_reduce(overlap, (1, block_size, 1))
            self.coarse_left_func = lambda overlap: block_reduce(overlap, (
            1, block_size, 1, 1)) if overlap.ndim == 4 else block_reduce(overlap, (block_size, 1, 1))
    ...
    def updateCanvas(self, inputPatchId, coord_X, coord_Y, blendTop=False, blendLeft=False, mode=None):
        ...
        if blendLeft:
            ...
            if mode == 'Cut':
                ...
                examplePatch_id[0][:, 0:self.overlapSize] = np.where(mask[..., 0], canvasOverlap_id,
                                                                     examplePatchOverlap_id)
            else:
                examplePatch[0][:, 0:self.overlapSize], mask = self.linearBlendOverlaps(canvasOverlap,
                                                                                        examplePatchOverlap, 'left')
                examplePatch_id[0][:, 0:self.overlapSize] = np.where(mask[..., 0], canvasOverlap_id,
                                                                     examplePatchOverlap_id)
        if blendTop:
            ...
            if mode == 'Cut':
                ...
                examplePatch_id[0][:self.overlapSize] = np.where(np.moveaxis(mask[..., 0], 0, 1), canvasOverlap_id,
                                                                 examplePatchOverlap_id)
            else:
                examplePatch[0][0:self.overlapSize, :], mask = self.linearBlendOverlaps(canvasOverlap,
                                                                                        examplePatchOverlap, 'top')
    ...
    def linearBlendOverlaps(self, canvasOverlap, examplePatchOverlap, mode):
        ...
        return canvasOverlap * (1 - mask) + examplePatchOverlap * mask, np.broadcast_to(np.array(mask < .5, dtype=bool),
                                                                                        canvasOverlap.shape)
...
if __name__ == '__main__':
    ...
    patchSize = int(H / 4)
    strict_match = True
    in_mirror_vert = False
    in_mirror_hor = False

    outputPath = f"{save_path}/{DATA_NAME}_{MODEL_NAME}/"
    if (H - patchSize) % 2 == 1:
        patchSize -= 1
    overlapSize = int((H - patchSize) / 2)

    pbts = patchBasedTextureSynthesis(patches, outputPath, outputSize, patchSize, overlapSize, in_windowStep=5,
                                      in_mirror_hor=in_mirror_hor, in_mirror_vert=in_mirror_vert, rotate=False,
                                      in_shapshots=True, picked_vertices=picked_vertices, patch_length=patch_length,
                                      coarse_KDtree=True, sample_tbn=data['patch_sample_tbn'], match_dim=match_dim,
                                      strict_match=strict_match, mode=mode)
    ...
    np.savez(pbts.outputPath + '/../texture.npz', features=pbts.canvas[..., :match_dim], mesh=None, grid_gap=grid_gap,
             sample_tbn=sample_tbn, sample_tbn_ids=canvas_id, phi_embed=phi_embed, local_tbn=local_tbn)
    print('Finish')
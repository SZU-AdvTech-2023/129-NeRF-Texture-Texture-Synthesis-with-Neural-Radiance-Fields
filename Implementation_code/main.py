...
if __name__ == '__main__':
    ...
    if not os.path.exists(surface_mesh_path):
        if surface_type == 'dach_reg' or surface_type == 'coacd_reg':
            ply_name = \
            sorted([x for x in os.listdir(opt.workspace + '/meshes/') if x.startswith('ngp_') and x.endswith('.obj')])[
                -1]
            ply_path = opt.workspace + '/meshes/' + ply_name
            decomp_path = DACH(mesh_path=ply_path, hull_num=hull_num) if surface_type == 'dach_remesh' else CoACD(
                mesh_path=ply_path, threshold=coacd_threshold)
            union_path = MeshUnion(mesh_path=decomp_path) if surface_type == 'dach_remesh' else MeshUnion_manifold(
                mesh_path=decomp_path)
    ...
    train_loader = NeRFDataset(opt, device=device, type='trainval', normalize=normalize,
                               optimize_camera=optimize_camera).dataloader()

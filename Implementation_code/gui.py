...
class NeRFGUI:
    def __init__(self, opt, trainer, gui_mode=True, debug=True):
        ...
        self.debug = debug
        self.bg_color = torch.ones(3, dtype=torch.float32).cuda()
        self.training = False
        self.step = 0
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            if self.gui_mode:
                outputs = self.trainer.test_gui(self.cam.pose, self.cam.intrinsics, self.W, self.H, self.bg_color,
                                                self.spp, self.downscale, self.simple_render)
            else:
    ...
    def render_round(self, render_light=False, fix_phi=False, fix_theta=False):
        ...
        sv_path = self.trainer.render_round(self.cam.intrinsics, 1920, 1080, self.bg_color, render_light=render_light,
                                            fix_phi=fix_phi, fix_theta=fix_theta)
            ...
            if not self.opt.test:
                with dpg.collapsing_header(label="Train", default_open=True):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="simple_render", tag="_button_simple_render",
                                       callback=callback_simple_render)
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="render_rphi", tag="_button_render_round_phi",
                                       callback=render_round_fixphi)
                        dpg.bind_item_theme("_button_render_round_phi", theme_button)
                        dpg.add_button(label="render_rtheta", tag="_button_render_round_theta",
                                       callback=render_round_theta)
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        def callback_unhash(sender, app_data):
                            if hasattr(self.trainer.model, 'meshfea_field') and hasattr(
                        ...
                        def callback_switch_import(sender, app_data):
                            ...
                            dpg.set_value("_log_field",
                                          f"Switch imported to {self.trainer.model.meshfea_field.imported}")
                            self.need_update = True

                        dpg.add_button(label="unhash", tag="_button_unhash", callback=callback_unhash)
                        dpg.bind_item_theme("_button_unhash", theme_button)
                        dpg.add_button(label="import_unhash", tag="_button_import_unhash",
                                       callback=callback_import_unhash)
                        dpg.bind_item_theme("_button_import_unhash", theme_button)
                        dpg.add_button(label="switch_shape_fea", tag="_button_switch_shape_fea",
                                       callback=callback_switch_shape_feature)
                    ...
                    with dpg.group(horizontal=False):
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            dpg.add_button(label='uv_rate', tag='_button_set_uv_rate',
                                           callback=callback_uv_utilize_rate)
                            ...
                            dpg.add_button(label='sdf factor', tag='_button_set_sdf_factor',
                                           callback=callback_sdf_factor)
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            dpg.add_button(label='sdf offset', tag='_button_set_sdf_offset',
                                           callback=callback_sdf_offset)
                            ...
                            dpg.add_button(label='set h_threshold', tag='_button_set_h_threshold',
                                           callback=callback_h_threshold)
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        for i in range(len(self.trainer.model.visual_modes)):
                            dpg.add_button(label=self.trainer.model.visual_modes[i], tag=f"_button_switch_visual_{i}",
                                           callback=create_callback_visual_mode(i))
                        ...
                        dpg.add_button(label="vis_fea", tag="_button_visual_features",
                                       callback=callback_visualize_features)
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="switch_light_model", tag="_button_switch_light_model",
                                       callback=callback_switch_light_model)
                        ...
                        for i in range(len(self.trainer.model.light_visual_modes)):
                            dpg.add_button(label=self.trainer.model.light_visual_modes[i],
                                           tag=f"_button_switch_light_visual_{i}",
                                           callback=create_callback_light_visual_mode(i))
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="switch_envmap", tag="_button_switch_envmap",
                                       callback=callback_switch_envmap)
                        ...
                        dpg.add_button(label="shade_visibility", tag="_button_switch_shade_visibility",
                                       callback=callback_switch_shade_visibility)
                        ...
                        def callback_switch_use_coarse_normal(sender, app_data):
                            if self.trainer.model.light_model == 'None':
                                ...
                                dpg.set_value("_light_model",
                                              f'use_coarse_normal: {self.trainer.model.use_coarse_normal}')
                                self.need_update = True

                        dpg.add_button(label="use_coarse", tag="_button_switch_use_coarse_normal",
                                       callback=callback_switch_use_coarse_normal)
                        ...
                        dpg.add_button(label="use_grad", tag="_button_switch_use_grad_normal",
                                       callback=callback_switch_use_grad_normal)

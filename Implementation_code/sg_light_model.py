...
#######################################################################################################
def render_with_sg(lgtSGs, specular_reflectance, roughness, diffuse_albedo, normal, viewdirs, blending_weights=None,
                   diffuse_rgb=None, normal_coarse=None):
    '''
    :param lgtSGs: [M, 7]
    ...
    if normal_coarse is None:
        ...
        lobe_prime_coarse, lambda_prime_coarse, mu_prime_coarse = lambda_trick(normal_coarse, lambda_cos, mu_cos,
                                                                               lobe_prime, lambda_prime, mu_prime)
        lobe_final_coarse, lambda_final_coarse, mu_final_coarse = lambda_trick(normal_coarse, lambda_cos, mu_cos,
                                                                               final_lobes, final_lambdas, final_mus)

        dotpc = torch.sum(lobe_prime_coarse * normal, dim=-1, keepdim=True)
        dotfc = torch.sum(lobe_final_coarse * normal, dim=-1, keepdim=True)

        specular_rgb = mu_prime_coarse * hemisphere_int(lambda_prime_coarse,
                                                        dotpc) - alpha_cos * mu_final_coarse * hemisphere_int(
            lambda_final_coarse, dotfc) - alpha_cos * mu_prime * hemisphere_int(lambda_prime, dot1) + (
                                   alpha_cos ** 2) * final_mus * hemisphere_int(final_lambdas, dot2)
    ...
    if diffuse_rgb is None:
        ...
        if normal_coarse is None:
            ...
            lobe_prime_coarse, lambda_prime_coarse, mu_prime_coarse = lambda_trick(normal_coarse, lambda_cos, mu_cos,
                                                                                   lobe_prime, lambda_prime, mu_prime)
            lobe_final_coarse, lambda_final_coarse, mu_final_coarse = lambda_trick(normal_coarse, lambda_cos, mu_cos,
                                                                                   final_lobes, final_lambdas,
                                                                                   final_mus)

            dotpc = torch.sum(lobe_prime_coarse * normal, dim=-1, keepdim=True)
            dotfc = torch.sum(lobe_final_coarse * normal, dim=-1, keepdim=True)

            diffuse_rgb = mu_prime_coarse * hemisphere_int(lambda_prime_coarse,
                                                           dotpc) - alpha_cos * mu_final_coarse * hemisphere_int(
                lambda_final_coarse, dotfc) - alpha_cos * mu_prime * hemisphere_int(lambda_prime, dot1) + (
                                      alpha_cos ** 2) * final_mus * hemisphere_int(final_lambdas, dot2)
...
class SG_EnvmapMaterialNet(nn.Module):
    ...
    def forward(self, geo_feat, normals, view_dirs, **kwargs):
        ...
        rgb, specular_rgb, diffuse_rgb, diffuse_albedo = render_with_sg(lgtSGs=sg_envmap_material['sg_lgtSGs'],
                                                                        specular_reflectance=sg_envmap_material[
                                                                            'sg_specular_reflectance'],
                                                                        roughness=sg_envmap_material['sg_roughness'],
                                                                        diffuse_albedo=sg_envmap_material[
                                                                            'sg_diffuse_albedo'],
                                                                        normal=normals, viewdirs=view_dirs,
                                                                        blending_weights=sg_envmap_material[
                                                                            'sg_blending_weights'],
                                                                        normal_coarse=None)
        return rgb, specular_rgb, diffuse_rgb, diffuse_albedo
    ...
    def load_envmap(self, path, log_path):
        if os.path.exists(self.specific_path(path) + '.npy'):
            ...
            self.lgtSGs_import = nn.Parameter(torch.from_numpy(np.load(self.specific_path(path) + '.npy')).cuda(),
                                              requires_grad=False)
        else:
            ...
            lgtSGs = EnvMap2SG(envmap=envmap, numLgtSGs=self.lgtSGs.shape[0], log_path=log_path, min_loss=5e-2,
                               sv_path=self.specific_path(path))
...
def EnvMap2SG(envmap, numLgtSGs=16, log_path='./logs/', min_loss=5e-2, sv_path='./logs/sg'):
    ...
    for step in range(N_iter):
        ...
        if step % 2000 == 0:
            ...
            save_envmap(gt_envmap_check, viewdirs, os.path.join(log_path, 'log_sg_{}_gt'.format(numLgtSGs)),
                        no_npz=True)

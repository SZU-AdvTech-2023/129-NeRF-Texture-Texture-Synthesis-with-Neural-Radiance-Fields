...
def MeshUnion_marching_cube(mesh_path, resolution=256, do_dilation=False):
    print('Calculate the unoin of watertight parts of ' + mesh_path + ' with resolution ' + str(
        resolution) + ' for marching cube.')
...
def Register(src_path, trg_path, save2trg=True, force=True, trg_is_ply=False):
    ...
    for _ in loop:
        ...
        if trg_is_ply:
            ...
            sample_trg = trg_verts[
                torch.from_numpy(np.random.choice(np.arange(trg_verts.shape[0]), [5000], replace=replace))]
...
def Align(src_path, trg_path):
    ...
    if format == 'msh':
        pymesh.save_mesh(save_path,
                         pymesh.form_mesh(vertices=src_vert_new, faces=src_mesh.faces, voxels=src_mesh.voxels))
    else:
...
def Direct_deform(src_path, style_path, hull_num=3):
    ...
    msh_arap_src_path, msh_arap_src_path_obj = ARAP_deform(msh_path=src_msh_path, src_template_path=src_union_path,
                                                           trg_template_path=reg_src_path, tmp_trg_scale=1.)
...
def InDirect_deform(src_path, style_path, template_path='./data/template/src_model.obj'):
    ...
    msh_arap_src_path, msh_arap_src_path_obj = ARAP_deform(msh_path=template_msh_path, src_template_path=template_path,
                                                           trg_template_path=reg_src_path, tmp_trg_scale=2.,
                                                           sv_path='/'.join(reg_src_path.split('/')[:-1]))
    ...
    msh_arap_style_path, msh_arap_style_path_obj = ARAP_deform(msh_path=template_msh_path,
                                                               src_template_path=template_path,
                                                               trg_template_path=reg_style_path, tmp_trg_scale=2.,
                                                               sv_path='/'.join(reg_style_path.split('/')[:-1]))
